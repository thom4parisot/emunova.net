...
function getEditLink(source_uri, next){
  if (/^(.+data.emunova.net)/.test(source_uri)) {
    return next.fn(source_uri.replace(RegExp.$1, "https://github.com/oncletom/data.emunova.net/tree/master"));
  }
  else {
    return next.inverse('#');
  }
