...
function getImages(system, namespace){
  var filename = "dist/data/"+(namespace || 'images')+"-"+ system.id +".json";
  return  grunt.file.exists(filename) ? grunt.file.readJSON(filename) : {};
}
...
function getGameImages(system, game, description_or_limit, next){
  var images = getImages.call(this, system, 'games-images')[game.id];
  var description = description_or_limit === true;
  var limit = parseInt(description_or_limit, 10) === description_or_limit ? description_or_limit : 0;
  var keys = Object.keys(images);
